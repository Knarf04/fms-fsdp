...
def main(**kwargs):
    ...
    cp_degree = 1  # GDN CP is not wired here; use test_gdn_cp.py for CP experiments

    # get model
    config_data = get_model_config(cfg.model_variant)
    # fla uses vocab_size; fms config_utils returns src_vocab_size
    if "src_vocab_size" in config_data:
        config_data["vocab_size"] = config_data.pop("src_vocab_size")
    gdn_config = GatedDeltaNetConfig(**config_data)
    if rank == 0:
        print(gdn_config)

    if cfg.low_cpu_fsdp:
        with torch.device("meta"):
            model = GatedDeltaNetForCausalLM(gdn_config)
    else:
        model = GatedDeltaNetForCausalLM(gdn_config)
    ...
    # FSDP2 — shard each GatedDeltaNetBlock and the token embedding independently.
    # fla model layout: model.model.embed_tokens (Embedding), model.model.layers[i]
    # (GatedDeltaNetBlock), model.lm_head (Linear).
    def lambda_fn(name, module):
        if isinstance(module, CheckpointWrapper):
            return True
        if isinstance(module, (GatedDeltaNetBlock, nn.Embedding)) or module is model.lm_head:
            if isinstance(module, GatedDeltaNetBlock) and "_checkpoint_wrapped_module" in name:
                return False
            return True
        return False
    ...
    for name, param in model.named_parameters():
        ...
        # No weight decay for biases and normalization weights (standard for transformers)
        is_no_decay = any(nd in name for nd in ("bias", ".norm.", "norm_f", "embed"))
